          ...
          <ConsolePanel>
            <div className="flex items-end justify-between gap-3">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-medium tracking-[-0.01em]">Pick the closest format.</h1>
              </div>
              ...
              {demoEventTemplates.map((item) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h2 className="font-medium text-venue-cream">{item.name}</h2>
                      <p className="mt-1 text-sm leading-6 text-venue-muted">{item.description}</p>
                    </div>
                    <span className="rounded-[999px] border border-venue-soft bg-white px-2.5 py-1 text-xs text-venue-dim">{item.eventType}</span>
                  </div>
              ...
              <div className="mt-3 grid gap-2">
                {(["Calm", "Social", "Mixer"] as VenueVibeLevel[]).map((level) => (
                  <button
                    ...
                    className={`rounded-[10px] border px-3 py-2 text-left text-sm ${vibeLevel === level ? "border-venue-cream bg-venue-raised" : "border-venue-soft bg-white"}`}
                    type="button"
                  >
                    <span className="font-medium text-venue-cream">{level}</span>
              ...
              <div className="mt-3 grid gap-2">
                {template.tables.slice(0, 3).map((table) => (
                  <p key={table.name} className="rounded-[10px] bg-venue-raised px-3 py-2 text-sm text-venue-muted">
                    <span className="font-medium text-venue-cream">{table.name}</span> / {table.prompt}
                  </p>
                ))}
              </div>
            </ConsolePanel>
